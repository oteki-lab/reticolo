...
%
% cao:transformée de coordonnees        [xinf,l+i*c]             [xinf,yinf,lx+i*cx,ly+i*cy]
...
%   init=                          {n,beta,si,d,cao,granet,cao_granet,parm}          {n,beta,ordre,alx,aly,nx,ny,si,d,cao,beta_sym,granet,cao_granetparm}
...
parm=struct('dim',2,'d',d,'cao',cao,'sog',sog,'delt',0,'sym',real(sym(1)),'nhanckel',N,'granet',0,'genre',1);
...
n1=ordre(1);n2=ordre(end);n=1+n2-n1;
beta=(2*pi/d)*[n1:n2];
if iscell(xl);% transformée de Granet
if length(xl{2})<4;xl{2}{4}=1;end;cao_granet=xl{2};cao_pasgranet=xl{1};[xl_granet,granet,sym,xl]=retgranet(xl{2}{:},sym,xl{1},d,n);
cao={[],xl_granet};
else;       % pas transformée de Granet
granet=[];cao_granet=[];    
...
parm=struct('dim',1,'d',d,'beta0',beta0,'sym',sym,'cao',xl,'granet',~isempty(granet),'sog',sog,'nsym',n,'nfourier',size(beta,2),'L',[],'genre',0);
if parm.granet==1;parm.cao_pasgranet=cao_pasgranet;end;
init={n,beta,si,d,cao,granet,cao_granet,parm};
end;
...
if iscell(xl);cao_granet=xl{2};xl=xl{1};granet=1;else;granet=0;end;% transformée de Granet
parm=struct('dim',2,'d',d,'beta0',beta0,'delt',delt,'sym',sym,'cao',xl,'sog',sog,'nsym',n,'nfourier',n,'L',L,'parm',struct('sym',sym,'cao',xl),'granet',granet,'genre',genre);
beta=beta+repmat(beta0.',1,n);
if ~isempty(sym)&sym(1)==0&sym(2)==0;sym=[];parm.sym=[];end;

if granet;% transformée de Granet
if length(cao_granet)<8;cao_granet=[cao_granet(1:3),{1},cao_granet(4:6),{1}];end;[xl_granet_x,xl_granet_y,granet,sym,xl]=retgranet(cao_granet{:},sym,xl,d,nx,ny);parm.cao_pasgranet=xl;
cao={{[],xl_granet_x},{[],xl_granet_y}};
else;       % pas transformée de Granet
granet=[];cao_granet=[];    
...
if sym(1)==0&sym(2)==0;si=[];n=2*n;init={n,beta,ordre,alx,aly,nx,ny,si,d,cao,[],granet,cao_granet,parm};return;end
...
init={n,beta,ordre,alx,aly,nx,ny,si,d,cao,beta_sym,granet,cao_granet,parm};
