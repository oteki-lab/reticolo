...
E=h*c./(wwavelength*1e-9)/q; %in eV flipud;s—ñ‚Ìã‰º‚ð”½“]
...
% J_tot=nansum((irr_eV(find(E>=E_G,1,'first'):end-1)./E(find(E>=E_G,1,'first'):end-1)).*(E(find(E>=E_G,1,'first')+1:end)-E(find(E>=E_G,1,'first'):end-1)));
% J_tot=nansum(
%(irr_eV(find(E>=E_G,1,'first'):end-1)@AM1.5[W.cm^-2.eV^-1]‚ÌE>E_G‚©‚çÅŒã‚Ìˆê—ñ‘O‚Ü‚Å
...
% end
wavelength=[];
text=['a.mat'];%ƒTƒ“ƒvƒ‹‚Æ‚µ‚Äd‚Ì\‘¢‚Ìƒtƒ@ƒCƒ‹
...
A{number}=1-R0_TE_TE;
A_GaAs{number}=Abs(3,:)+Abs(5,:)+Abs(4,:);%GaAs(QD‘w)‚ÌŽw’è
A_front_AlGaAs{number}=Abs(2,:);%‘‹‘w‚ÌŽw’è
A_back_AlGaAs{number}=Abs(6,:);%BSF‘w‚ÌŽw’è
A_Ag{number}=Abs(7,:);

% A_GaAs{number}=Abs(3,:)+Abs(5,:)+Abs(4,:);%GaAs(QD‘w)‚ÌŽw’è
% A_front_AlGaAs{number}=Abs(2,:);%‘‹‘w‚ÌŽw’è
% A_back_AlGaAs{number}=Abs(6,:)+Abs(7,:)+Abs(8,:)+Abs(9,:);%BSF‘w‚ÌŽw’è
% A_Ag{number}=Abs(10,:);
