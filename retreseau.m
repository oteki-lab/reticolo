...
% marche aussi avec retfp. amplitude a alors une dimension de plus.
%        il faut executer: ef=retreseau(init,s_retfp,betb,cb,anglb,incb,difb,beth,ch,anglh,inch,difh);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
ef=struct('inc',inc,'dif',dif,'amplitude',s);LL=size(s,3);% pour retfp modif 3 2014
...
prv=zeros(size(ef.amplitude,1)+length(k),size(ef.amplitude,2),LL);for ll=1:LL;prv(:,:,ll)=[ef.amplitude(:,:,ll);diag(sym(2).*exp(2i*ef.dif.beta(2,k)*sym(4)))* ef.amplitude(k,:,ll)];end;ef.amplitude=prv;
...
ef.amplitude(k,:,:)=ef.amplitude(k,:,:)/sqrt(2);    
prv=zeros(size(ef.amplitude,1)+length(k),size(ef.amplitude,2),LL);for ll=1:LL;prv(:,:,ll)=[ef.amplitude(:,:,ll);-diag(sym(1).*exp(2i*ef.dif.beta(1,k)*sym(3)))*ef.amplitude(k,:,ll)];end;ef.amplitude=prv;    
ef.dif.teta=[ef.dif.teta,ef.dif.teta(k)];    
...
% remplissage de 'h' 'b' 'TE'  'TM'  'TMh'  'TMb'  'TEh'  'TEb'
...
for ll=1:LL;ef.amplitude(:,:,ll)=ef.amplitude(:,:,ll)*cc;end;
...
for ll=1:LL;ef.amplitude(:,:,ll)=diag(1./c)*ef.amplitude(:,:,ll);end;
...
ef.amplitude(k,:,:)=ef.amplitude(k,:,:)/sqrt(2);    
prv=zeros(size(ef.amplitude,1)+length(k),size(ef.amplitude,2),LL);for ll=1:LL;prv(:,:,ll)=[ef.amplitude(:,:,ll);diag(sym(1).*exp(2i*ef.dif.beta(1,k)*sym(2)))*ef.amplitude(k,:,ll)];end;ef.amplitude=prv;
ef.dif.teta=[ef.dif.teta,-ef.dif.teta(k)];    
