...
nb_lambda=401; %Number of wavelength
lambda=linspace(0.4,1.2,nb_lambda)*1e-6; %Wavelength vector
...
n_end=retindice_chen(lambda*1e6,1.72); %Last medium
...
n(1,:)=1.85*ones(1,nb_lambda);
% n(1,:)=3.5+0.01i*ones(1,nb_lambda);
n(2,:)=retindice_chen(lambda*1e6,4.802);
n(3,:)=retindice_chen(lambda*1e6,4.707);
n(4,:)=retindice_chen(lambda*1e6,4.708);
n(5,:)=retindice_chen(lambda*1e6,4.707);
n(6,:)=retindice_chen(lambda*1e6,4.802);
n(7,:)=retindice_chen(lambda*1e6,1.72);
