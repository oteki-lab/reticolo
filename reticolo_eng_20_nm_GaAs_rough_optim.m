...
Nb_couches=8; %Number of layers (between 1 and 12)
...
% wavelength=1./wavelength_inv;
theta=[0,0];                          %angle of incidence in degrees

period=0.40:0.05:0.60;
diam_perc=0.5:0.1:0.8;
height=0.1:0.05:0.4;
ii=1;
jj=1;
kk=1;
for number=1:length(period)*length(diam_perc)*length(height)
    sequence{number}=[period(ii),diam_perc(jj),height(kk)];
    if kk<length(height)
        kk=kk+1;
    else
        kk=1;
...
for i=1:length(sequence)
    ...
    periodicity_x=sequence{i}(1);                  % period in x
    periodicity_y=periodicity_x;        % period in y
    diameter_x=periodicity_x*sequence{i}(2);                       % structure size in x
    diameter_y=diameter_x;              % structure size in y
    
    % Thicknesses, from top to bottom
    h1=0.090;                           % Thickness of layer 1 (0 si if no layer)
    h2=0.060;                           % Thickness of layer 2 (0 si if no layer)
    h3=0.025;                           % Thickness of layer 3 (0 si if no layer)
    h4=0.020;                           % Thickness of layer 4 (0 si if no layer)
    h5=0.025;                           % Thickness of layer 5 (0 si if no layer)
    h6=0.010;                               % Thickness of layer 6 (0 si if no layer)
    h7=sequence{i}(3);                               % Thickness of layer 7 (0 si if no layer)
    h8=0.150;                               % Thickness of layer 8 (0 si if no layer)
    h9=0;                               % Thickness of layer 9 (0 si if no layer)
    h10=0;                              % Thickness of layer 10 (0 si if no layer)
    h11=0;                              % Thickness of layer 11 (0 si if no layer)
    h12=0;                              % Thickness of layer 12 (0 si if no layer)
    
    %%%%%% Refraction indices (from top to bottom), can be a function of the wavelength
    nh=1;                                % index of incident medium
    n1=retindice_dielec(wavelength,23.31);       % index between the structures of layer 1
    n1m=0;      % index inside the structures of layer 1 (0 if not structured)
    n2=retindice_dielec(wavelength,23.5);  % index between the structures of layer 2
    n2m=0;                               % index inside the structures of layer 2 (0 if not structured)
    n3=retindice_semicond(wavelength,4.605);  % index between the structures of layer 3
    n3m=0;                               % index inside the structures of layer 3 (0 if not structured)
    n4=retindice_semicond(wavelength,41.01); % index between the structures of layer 4
    n4m=0;                               % index inside the structures of layer 4 (0 if not structured)
    n5=retindice_semicond(wavelength,4.605);   % index between the structures of layer 5
    n5m=0;                               % index inside the structures of layer 5 (0 if not structured)
    n6=1.9*ones(length(wavelength));                                % index between the structures of layer 6
    n6m=0;                               % index inside the structures of layer 6 (0 if not structured)
    n7=1.9*ones(length(wavelength));                                % index between the structures of layer 7
    n7m=retindice_metal(wavelength,1.9);                               % index inside the structures of layer 7 (0 if not structured)
    n8=retindice_metal(wavelength,1.9);                                % index between the structures of layer 8
    n8m=0;                               % index inside the structures of layer 8 (0 if not structured)
    n9=1;                                % index between the structures of layer 9
    n9m=0;                               % index inside the structures of layer 9 (0 if not structured)
    n10=1;                               % index between the structures of layer 10
    ...
    nsub=ones(length(wavelength)); % index of the substrate
    ...
    N_semicon=4;                         % Layer where field is calculated (if cal_champ=1)
    ...
    parfor zou=1:length(wavelength)
        ...
        for az=1:Nb_couches
            if Nm(az)==0
                ...
                u{az}=retu(period,{N(az),[0,0,diameter_x,diameter_y,Nm(az),Ntre],k0});
            end;
    ...
    % AbsRCWA=1-R0_TE_TE;
    text=['AbsRCWA_20nm_GaAs','wav',int2str(wavelength(1)*1000),'_',int2str(wavelength(end)*1000),'_period_',int2str(periodicity_x*1000),'_diam_',int2str(diameter_x*1000),'_height_',int2str(h7*1000),'_Fourier',int2str(Mx),'_nbpoints',int2str(length(wavelength)),'_R_only.mat'];
%     text='AbsRCWA_20nm_GaAs_check_Andrea.mat';
    ...
    figure
    % plot(wavelength,Abs(1,:),'Linewidth',3)
    hold on
%     plot(wavelength,Abs(3,:),'Linewidth',3)
%     plot(wavelength,Abs(4,:),'Linewidth',3)
%     plot(wavelength,Abs(5,:),'Linewidth',3)
%     plot(wavelength,Abs_plots(7,:)+Abs(8,:),'Linewidth',3)
    plot(wavelength,1-R0_TE_TE,'Linewidth',3,'Linestyle','-.')
    xlabel('\lambda (É m)')
    ylabel('Absorption')
    xlim([min(wavelength) max(wavelength)])
    ylim([0 1])
    set(gca,'Fontsize',12)
%     legend({'Front AlGaAs','GaAs','Back AlGaAs','Silver mirror','Total'})
