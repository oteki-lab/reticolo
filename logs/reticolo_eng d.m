...
n6=retindice_chen(wavelength,4.708); % gaas index between the structures of layer 4
n6m=0;    % argent index inside the structures of layer 4 (0 if not structured)
n7=retindice_chen(wavelength,4.707);   % AlInP argentindex between the structures of layer 5
n7m=0; 
...
op_objet=0;                          % If 1, plot the geometry to verify the calculated structure is correct
 ...
 diameter_y=diameter_x;
for az=1:Nb_couches
    if op_granet==1;Bx=500;Ax=0.02/Bx;By=Bx;Ay=Ax;xdisc=[-unique(diameter_x)/2,unique(diameter_x)/2];ydisc=[-unique(diameter_y)/2,unique(diameter_y)/2];
    end
end
...
parfor zou=1:length(wavelength)
    ...
    ref_TM_TM(:,zou)=ref_TM_TM_vect;
        ...
        [Xdisc,Ydisc]=retgranet(init,[-unique(diameter_x)/2,unique(diameter_x)/2],[-unique(diameter_y)/2,unique(diameter_y)/2]);
    ...
    if trace_champ==1
        ...
        [xx,wx]=retgauss(-periodicity_x/2,periodicity_x/2,15,12,[-unique(diameter_x)/2,unique(diameter_x)/2]);
        [yy,wy]=retgauss(-periodicity_y/2,periodicity_y/2,15,12,[-unique(diameter_y)/2,unique(diameter_y)/2]);
...
figure
plot(wavelength,Abs(7,:)+Abs(5,:)+Abs(6,:),'Linewidth',3)
%hold on 
