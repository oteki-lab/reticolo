...
%  function [w,xdisc]=retu(init,x,epx,y,epy,ww);
%
%  init (obtenu par retinit) peut etre remplace par d le pas(‘«ÕH) du reseau(–Ô)
%init(retinit‚©‚ç“¾‚ç‚ê‚é) ‚ÍŠiŽqŠÔ‹——£d‚ÆŽæ‚èŠ·‚¦‚ç‚ê‚é
%
%  CAS 1D 
%  [w,xdisc]=retu(init,x,ep); 
%  x discontinuitees de x (croissant(‘‰Á))  ,ep: valeurs de ep (calcules par retep) a gauche(¶) de x 
%x:‘‰Á‚·‚éx‚Ì•s˜A‘±«,ep:
% (en cas de depassement prioritee a gauche)
%  remplissage(–ž‚½‚·) de  w={x ep} utilisees par retcouche xdisc  points de discontinuite en x (modulo d)
%
%   milieu(^‚ñ’†) homogene(‹ÏŽ¿): w=retu(ep)
%
%  CAS 2D   
...
%  si ww n'existe pas:on  remplit(–ž‚½‚·) w  
%  si ww existe on  SUBSTITUE uu a u QUAND uu~=0
%   l'objet (ou(‚ ‚é‚¢‚Í) la variation de l'objet) est le produit d'une fonction de x par une fonction de y
...
%
%   calcul du maillage(ŠiŽq‚Ì‘å‚«‚³) de texture w pour les metaux infiniment conducteurs:
%   w={x,dx,eps,pol,mm} est un cell array  decrivant(•`ŽÊ) l'objet
%  
