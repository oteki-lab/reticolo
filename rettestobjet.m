...
%    rettestobjet(d,w,[],[2,-d/2]);%   trace recentre en x
%
%% EXEMPLES 2 D
%% Maillage de texture
...
switch(calo(1));
case i;ee=retsqrt(prod( retsqrt(ee(:,:,[1,2,4,5])),3));
case -i;ee=retsqrt(ee(:,:,4)./ee(:,:,1));
...
% h(ii)=retisosurface(X,Z,Y,oo); % modif 9 2011
h(ii)=retisosurface(X,Y,Z,oo);  % modif 9 2011
...
axis(ax);
%xlabel('x');ylabel('z');zlabel('y');% modif 9 2011
xlabel('x');ylabel('y');zlabel('z');% modif 9 2011
